...
# vLLM engine parameters (overridable by command line)
DEFAULT_MAX_MODEL_LEN = 81920
...
def build_user_content_item(
    ...
    if data_type == "video":
        ...
        video_item2: Dict[str, Any] = {
            "type": "video",
            "video": full_path,
            "max_pixels": max_pixels_video,
        }
        if nframes > 0:
            video_item2["nframes"] = nframes
        else:
            video_item2["fps"] = fps
            video_item2["max_frames"] = max_frames
        content.append(video_item2)
...
def main():
    ...
    parser.add_argument("--nframes", type=int, default=0,
                        help="If >0, use fixed nframes (ignoring fps/max_frames). "
                             "Each video will be uniformly sampled to exactly this many frames.")
    ...
    parser.add_argument("--max_model_len", type=int, default=DEFAULT_MAX_MODEL_LEN,
                        help="vLLM max_model_len. Lower this (e.g. 32768) on small-VRAM GPUs.")
    parser.add_argument("--gpu_memory_utilization", type=float, default=DEFAULT_GPU_MEM_UTIL,
